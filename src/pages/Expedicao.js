...
class Expedicao extends Component{
  ...
  render(){
    ...
    const list = all?.value?.filter(a => a.fase === '14 - EXPEDICAO')
    all?.value?.shift()
    const prd = all?.value?.filter(a => a.fase === '14 - EXPEDICAO')
        ...
        <div>
          ...
          {list?.map(({fase, produto,qtde}) => (
            <ul key={fase}>
              ...
              <h3 style={{borderStyle: 'solid' ,marginTop: '15px', marginBottom:'200px', width: 'fit-content', color: 'Black', backgroundColor: '#15CDFC'}}> {prd?.map(({produto}) => (
